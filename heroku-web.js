...
function add_prayers(user, points, callback) {//get user object and points to add and return new user updated object,new level flag
...
app.post('/get_top_5', function (req, res) {
    User.find({ "admin": false }).sort({ "score": -1 }).limit(5).exec(function (err, users) {
        if (err) {
            res.send(err);
        } else {
            res.send(users);
